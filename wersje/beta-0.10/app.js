...
const RELEASE_NOTES = {
  ...
  changes: [
    ...
    'Trening siłowy: stoper odpoczynku (zwijany, na dole) i czas treningu zapisywany do wpisu',
    'Poprawiony kalendarz i odznaki — nie rozjeżdżają się na wąskich ekranach'
  ]
};
...
function applyTheme(theme) {
  ...
  if (meta) meta.setAttribute('content', theme === 'light' ? '#ffffff' : '#0f0f13');
  const d = document.getElementById('theme-dark');
  const l = document.getElementById('theme-light');
  if (d) d.classList.toggle('active', theme !== 'light');
...
function paceText(duration, distance) {
  if (duration > 0 && distance > 0) return formatPace(duration / 60 / distance) + ' /km';
  return '';
}
...
function showTab(name) {
  ...
  if (name === 'kalendarz') { renderCalendar(); renderDayDetail(); }
...
function renderDayDetail() {
  ...
  if (!state.dayDate) { wrap.innerHTML = ''; return; }
  ...
  logs.forEach(l => {
    ...
    html += entryBox('gym', esc(l.title), ex + kcalHtml(l.kcal), l.notes, 'log', l.id);
  });
  runs.forEach(r => {
...
function entryBox(tag, title, body, notes, kind, id) {
  return '<div class="entry"><div class="entry-head">' +
    ...
    '<span class="e-tag ' + tag + '">' + (tag === 'gym' ? 'Siłownia' : tag === 'run' ? 'Bieganie' : 'Zdrowie') + '</span>' +
...
function renderTraining() {
  ...
  if (c.category === 'silownia') {
    ...
    c.exercises.forEach((ex, i) => {
      ...
      html += '<div class="ex-block"><div class="ex-head">' +
        '<span class="ex-name">' + (i + 1) + '. ' + esc(ex.name) + '</span>' +
        '<button class="mini-btn" data-act="del-ex" data-ex="' + i + '">Usuń</button></div>';
      if (last) {
    ...
    html += '<div class="form-row"><div><label class="field-label">Czas (min)</label>' +
      '<input id="run-duration" type="number" step="any" inputmode="decimal" value="' + esc(c.duration) + '" placeholder="np. 45"></div>' +
...
function updateRunStats() {
  ...
  const t = num(document.getElementById('run-duration').value);
...
const workoutClock = { seconds: 0, timer: null };
...
function workoutClockTick() {
  ...
  if (el) {
    el.textContent = fmtClock(workoutClock.seconds);
    el.classList.toggle('running', true);
  }
}

function toggleWorkoutClock() {
  if (workoutClock.timer) {
    clearInterval(workoutClock.timer);
    workoutClock.timer = null;
  } else {
    workoutClock.timer = setInterval(workoutClockTick, 1000);
    workoutClockTick();
  }
  const el = document.getElementById('wt-toggle');
  if (el) el.textContent = workoutClock.timer ? 'Stop' : 'Start';
  const t = document.getElementById('wt-time');
  if (t) t.classList.toggle('running', workoutClock.timer !== null);
}
...
function workoutTimerHtml() {
  return '<div class="workout-timer">' +
    '<div class="wt-label">' + icon('clock') + ' Czas treningu</div>' +
    '<div class="wt-time" id="wt-time">' + fmtClock(workoutClock.seconds) + '</div>' +
    '<div class="wt-actions"><button class="btn primary" id="wt-toggle">' + (workoutClock.timer ? 'Stop' : 'Start') + '</button>' +
    '<button class="btn secondary" id="wt-reset">Wyzeruj</button></div></div>';
}
...
function saveTraining() {
  ...
  if (c.category === 'silownia') {
    const exercises = c.exercises
      .map(ex => ({ name: ex.name, sets: ex.sets.filter(s => s.w !== '' || s.r !== '') }))
    ...
    if (state.editLogId) {
      ...
      if (log) {
        ...
        if (workoutClock.seconds >= 30) log.duration = Math.round(workoutClock.seconds / 60);
      }
    } else {
      data.logs.push({ id: uid(), date: date, planId: c.planId, title: title, kcal: kcal, notes: notes, exercises: exercises, tags: c.tags.slice(), duration: workoutClock.seconds >= 30 ? Math.round(workoutClock.seconds / 60) : 0 });
    }
    save();
    toast('Zapisano trening siłowy');
    resetWorkoutClock();
  } else {
    const duration = num(document.getElementById('run-duration').value);
  ...
  resetCurrent();
  showTab('kalendarz');
  if (state.dayDate) renderDayDetail();
}
...
function renderHealth() {
  ...
  if (!sorted.length) html += '<div class="chart-empty">Brak pomiarów. Dodaj pierwszy powyżej.</div>';
    ...
    html += '<div class="health-list">' + sorted.map(h => {
      ...
      return '<div class="health-list-item"><span class="h-date">' + shortDate(h.date) + '</span>' +
        '<span class="h-vals">' + esc(vals.join(' · ')) + '</span>' +
        '<span class="entry-actions"><button class="mini-btn" data-edit="health:' + h.id + '">Edytuj</button>' +
        '<button class="mini-btn" data-del="health:' + h.id + '">Usuń</button></span></div>';
    }).join('') + '</div>';
  }
  html += '</div>';
...
function renderPhotos() {
  ...
  html += '<div class="photo-upload"><input type="file" id="photo-file" accept="image/*" capture="environment">' +
  ...
  if (!sorted.length) html += '<div class="chart-empty">Brak zawodów. Dodaj pierwszy powyżej.</div>';
    ...
    html += '<div class="race-list">' + sorted.map(r => {
      ...
      return '<div class="race-item' + (r.date < today ? ' past' : '') + '">' +
        ...
        '<span class="race-status ' + (r.status === 'ukonczony' ? 'fin' : '') + '">' + st.label + '</span>' +
        '<span class="entry-actions">' +
...
function bindEvents() {
  ...
  document.getElementById('health-form').addEventListener('click', e => {
    ...
    if (dl) {
      const [kind, id] = dl.dataset.del.split(':');
      if (kind === 'health') deleteEntry('health', id);
    }
  });
